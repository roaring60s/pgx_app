...
# /media/p/D11/patients/B020_20241127/asa_7C_combined_19C_020450_ST6.txt
...
def fnc_drug_list_step9(file1, pref1, pref2, suf1, suf2):
    with open(file1, "r")as px:
        ...
        for lnx in px:
            ...
            drug_list.clear()
            print(f"STEP 8, Generating Drug List {counter}, {pgx_idx}")
        ...
        for lnx in px:
            ...
            with open(path_in, "r") as p1:
                for ln1 in p1:
                    ...
                    # score_dv = sum(gt_ls) // len(gt_ls)
                    
                    # The ClinPharm score already factors 4 characteristics: allele function, P value, cohort size, and effect.
                    
                    if any(item >= 280 for item in res_ls):
                        weight = 3
                        print(s.join(ls1), weight, sep=s, file=fileout)
                        # break
                    elif any(item > 100 for item in res_ls):
                        weight = 2
                        print(s.join(ls1), weight, sep=s, file=fileout)
                        # break
                    elif any(item > 2 for item in res_ls):
                        weight = 1
                        print(s.join(ls1), weight, sep=s, file=fileout)
                            
                    # if score_average >= 700:
                    #     weight = 3
                    #     # noinspection PyTypeChecker
                    #     print(s.join(ls1), weight, sep=s, file=fileout)
                    #
                    # elif 300 <= score_average < 700:
                    #     weight = 2
                    #     # noinspection PyTypeChecker
                    #     print(s.join(ls1), weight, sep=s, file=fileout)
                    #
                    # if 1.5 <= score_average < 300:
                    #     weight = 1
                    #     print(s.join(ls1), weight, sep=s, file=fileout)
